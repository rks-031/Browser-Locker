        ...
        if (localStorage.getItem("WrongAttempt") != "true") {
          ...
          kalanhak.kalan().then((donus) => {
            if (donus <= 0) {
              util.uyarigonder(
                "opps!",
                util.ceviri("deneme_kaldi_bir") +
                  donus +
                  util.ceviri("deneme_kaldi_iki")
              );
              kalanhak.kilitle();
            } else {
              util.uyarigonder(
...
$(document).on("mouseup mousedown mouseleave", ".show_password", function (e) {
  let sclass =
    e.type == "mousedown"
      ? "text"
      : e.type == "mousedown" || e.type == "mouseleave"
      ? "password"
      : "password";
  ...
  if (util.ValidMail(localStorage.getItem("MainMail")) == false) {
    ...
    if (hkljdm == "false") {
      ...
      login.recaptcha().then((resolve, reject) => {
        util.AnalyticsEvent(
          "sifre_recovery",
          "sifre recovery has been started"
        );
        if (resolve == "true") {
          $(".pass_root, #" + event.target.id)
            .toggleClass("kapali")
            .trigger("ackapa");
        ...
        if (npass.val().length > 0 && npass.val().length <= 25) {
          try {
            util.StorageSet("user__pass", md5(npass.val())).then(() => {
              util.onaygonder(
                "Browser Lock",
                util.ceviri("sifre_ok_degisti") + npass.val()
              );
          ...
          util.uyarigonder(
            "Browser Lock",
            util.ceviri("sifre_uzunluk_kontrol")
          );
        ...
        login.KodDonder(userMail).then((c, reject) => {
          if (c["hata"] != false) {
            ...
            switch (c.hata) {
              case 1:
                uyari =
                  "Something went wrong with mail adress. Please try again later.";
                break;
              case 2:
                uyari =
                  "Something went wrong with mail adress. Please try again later.";
              ...
              case 3:
                uyari =
                  "Something went wrong with recaptcha. Please try again later.";
                break;
              case 4:
                uyari =
                  "Something went wrong with mail sending. Please try again later. If this keep happining please contact with me on humbldump@protonmail.com";
                break;
              case 5:
                uyari =
                  "Something went wrong with recaptcha. Please restart your browser.";
                break;
              default:
