...
$("input[type='checkbox']").on("click", function (e) {
  ...
  option
    .switchSonuc(this.id, $(this).is(":checked"))
    .then((resolve, reject) => {
      if (resolve === "true") {
          ...
          if (oldPassHash === sonuc) {
            if ($(newInput).val() === $(newaInput).val()) {
              if ($(newInput).val() != $(oldInput).val()) {
                ...
                util.uyarigonder(
                  uygulamaBaslik,
                  util.ceviri("eski_yeni_sifre_ayni")
                );
              }
            } else {
              ...
              util.uyarigonder(
                uygulamaBaslik,
                util.ceviri("yeni_sifre_eslesmedi")
              );
            }
          } else {
  ...
  switch (Object.values(e.currentTarget.classList)[1]) {
    case "notifyjs-uyar-gizli":
      chrome.tabs.query({}, (tabs) => {
        const tabara = tabs.find(
          (tab) => tab.url.indexOf("chrome://extensions") !== -1
        );
        if (tabara) {
...
$(document).on(
  "click",
  ".mail_button_section button#btn__field.btn-1e.main__mail__btn",
  function maildegis() {
    if (
      (mail = util.ValidMail(
        window.prompt(
          alert(util.ceviri("epostani_gir")),
          "e.g. humbldump@protonmail.com"
        )
      ))
    ) {
...
class option {
      ...
      switch (elm) {
        ...
        case "browser__lock__switch":
          if (iliski == true) {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("tarayici_kilit") + util.ceviri("acik")
            );
          } else {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("tarayici_kilit") + util.ceviri("kapali")
            );
          }
          break;
        //Karanlık Mod Ayarı
        case "dark__mode__switch":
          if (iliski == true) {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("karanlik_mod") + util.ceviri("acik")
            );
          } else {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("karanlik_mod") + util.ceviri("kapali")
            );
          }
          break;
        ...
        case "password__recovery__switch":
          if (iliski == true) {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("sifremi_unuttum") + util.ceviri("acik")
            );
          } else {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("sifremi_unuttum") + util.ceviri("kapali")
            );
          }
          break;
        ...
        case "password__attempt__switch":
          if (iliski == true) {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("derin_guvenlik") + util.ceviri("acik")
            );
          } else {
            util.onaygonder(
              uygulamaBaslik,
              util.ceviri("derin_guvenlik") + util.ceviri("kapali")
            );
          }
          break;
        ...
        case "clear__history__switch":
          if (localStorage.getItem("WrongAttempt") != "true") {
            ...
            if (iliski == true) {
              util.onaygonder(
                uygulamaBaslik,
                util.ceviri("gecmisi_temizle") + util.ceviri("acik")
              );
            } else {
              util.onaygonder(
                uygulamaBaslik,
                util.ceviri("gecmisi_temizle") + util.ceviri("kapali")
              );
            }
          }
          break;
  ...
  static MailKontrol() {
    if (util.ValidMail(localStorage.getItem("MainMail")) == false) {
      ...
      var mail = window.prompt(
        util.ceviri("epostani_gir"),
        "e.g. humbldump@protonmail.com"
      );
    ...
    $(".email__show_")
      .text(localStorage.getItem("MainMail"))
      .parent()
            ...
            save: {
              ...
              action: function () {
                ...
                try {
                  ...
                  if (md5(Sifre.val()) != pass)
                    throw util.ceviri("sifre_yanlis");
            ...
            action: function () {
              ...
              if (Sifre.val().length > 0 && Sifre.val().length <= 25) {
                ...
                if (Sifre.val() === Sifret.val()) {
                  try {
                    util
                      .StorageSet("user__pass", md5(Sifre.val()))
                      .then(() => {
